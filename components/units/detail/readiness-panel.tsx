...
import { useUnitReadiness } from "@/hooks/use-cockpit"
 ...
 * `evaluateUnitReadiness` und gelten später serverseitig genauso. Welche Punkte
 * überhaupt gelten, entscheidet der Bereich: Ein Gerät ohne Zulassungspflicht
 * braucht keine ABE.
 */
export function ReadinessPanel({ unit }: { unit: ArticleUnit }) {
  ...
  if (unit.saleStatus === "VERKAUFT") return null
  ...
  async function setReady() {
    ...
    toast.success(`${unit.unitNumber} ist verkaufsbereit`, {
      description: "Das Gerät kann jetzt auf den Kanälen veröffentlicht werden.",
    })
  }
